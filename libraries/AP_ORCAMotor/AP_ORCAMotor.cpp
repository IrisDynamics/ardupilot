...
void AP_ORCAMotor::init(AP_HAL::UARTDriver *uart){
    motor_uart = uart;
    if (uart == nullptr) {
...
void AP_ORCAMotor::enqueue_ping_message(){
...
void AP_ORCAMotor::enqueue_extended_motor_frame(uint32_t position_um, uint16_t read_address){
    position_mode = 3;
...
bool AP_ORCAMotor::check_ping_response(){
...
bool AP_ORCAMotor::check_motor_frame_response()
{
...
bool AP_ORCAMotor::bad_crc(uint8_t* rx_data, uint16_t rx_message_length){
...
uint16_t AP_ORCAMotor::rx_buffer_count()
{
    if (rx_head >= rx_tail)
